...
// 祭司探图定时:5分钟(7500 tick)~8分半(12750 tick)为强制探图窗口,
    ...
    if (1) {
        ...
        if(timer>=allOut_time&&!allOut_started)allOut_started=true;  // 触发总攻,同时停用旧敌袭/回家/探图逻辑
        
        // auto pickDirection=[&](tagArmy& army){
        //     int lastDir=allOut_dir[army.SN];
        //     double bestScore=0;
        //     int bestDir=0;
        //     int curDR=army.BlockDR;
        //     int curUR=army.BlockUR;
        //     for(int d=0;d<8;d++){
        //         if(d==lastDir)continue;
        //         double score=0;
        //         int unknownBlockCnt=0;
        //         int newDR=curDR;
        //         int newUR=curUR;
        //         int pace=1;
        //         while(newDR>=0&&newDR<MAP_L&&newUR>=0&&newUR<MAP_U){
        //             newDR=curDR+diri[d]*pace;
        //             newUR=curUR+dirj[d]*pace;
        //             if(curMap[newDR][newUR]==-1)unknownBlockCnt++;
        //             pace++;
        //         }
        //         //权重问题暂时搁置
        //         score+=unknownBlockCnt;
        //         score+=abs(baseBlockDR-newDR)+abs(baseBlockUR-newUR);
        //         if(score>bestScore){
        //             bestScore=score;
        //             bestDir=d;
        //         }
        //     }
        //     allOut_dir[army.SN]=bestDir;
        // };
        // auto allOut_move=[&](tagArmy& army){
        //     int dir = allOut_dir[army.SN];
        //     int newDR=army.BlockDR+diri[dir]*4;
        //     int newUR=army.BlockUR+dirj[dir]*4;
        //     if(newDR<3||newDR>=98||newUR<3||newUR>=98||
        //     curMap[army.BlockDR+diri[dir]][army.BlockUR+dirj[dir]]==1||
        //     curMap[army.BlockDR+diri[dir]][army.BlockUR+dirj[dir]]==2||
        //     curMap[army.BlockDR+diri[dir]][army.BlockUR+dirj[dir]]==4){
        //         pickDirection(army);
        //         dir = allOut_dir[army.SN];
        //         newDR=army.BlockDR+diri[dir]*4;
        //         newUR=army.BlockUR+dirj[dir]*4;
        //     }
        //     if(newDR<0||newDR>=100||newUR<0||newUR>=100)return;
        //     HumanMove(army.SN,army.DR+diri[dir]*BLOCKSIDELENGTH*4,army.UR+dirj[dir]*BLOCKSIDELENGTH*4);
        // };
        // if(allOut){
        //     for(tagArmy& army:info.armies){
        //         if(!allOut_dir.count(army.SN)){
        //             allOut_dir[army.SN]=-1;
        //             pickDirection(army);
        //         }
        //         if((timer+army.SN)%40==0){
        //             allOut_move(army);
        //         }
        //     }
        // }
